...
import 'custom_profile_popup.dart';
        ...
        if (snapshot.data == null || !isSuggestionExpanded) {
          ...
          final Widget listView = ListView.builder(
            ...
            itemBuilder: (context, index) => TextFieldTapRegion(
                child: InkWell(
                  onTap: () {
                    searchController!.text = snapshot.data![index]!.searchKey.split("*").first;
                  ...
                  child: Container(
                      ...
                      child: snapshot.data![index]!.child ??
                          ListTile(
                            ...
                            leading: snapshot.data![index]!.searchKey.split("*").last.contains("FileUpload")?InkWell(
                                onTap: (){
                                  customProfileDialog(context,snapshot.data![index]!.searchKey.split("*").last,snapshot.data![index]!.searchKey.split("*").first.split("/").first);
                                },
                                child: CircleAvatar(backgroundImage: NetworkImage(snapshot.data![index]!.searchKey.split("*").last))):const Icon(Icons.account_circle, size: 33),
                          )
                  ),
                )),
          );
